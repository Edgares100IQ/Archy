...
menu_desinstalar_navegador() {
    while true; do
        ...
        read -p "$MSG_CHOOSE" op
        case "$op" in
            1) sudo pacman -Rns firefox --noconfirm; break ;;
            2) sudo pacman -Rns chromium --noconfirm; break ;;
            3) yay -Rns brave-bin --noconfirm; break ;;
            4) yay -Rns opera --noconfirm; break ;;
            5) yay -Rns tor-browser-bin --noconfirm; break ;;
            6) yay -Rns zen-browser-bin --noconfirm; break ;;
            7) break ;;
            *) echo "$MSG_INVALID" ;;
        esac
    done
...
menu_desinstalar_editor() {
    while true; do
        ...
        read -p "$MSG_CHOOSE" op
        case "$op" in
            1) yay -Rns vscodium-bin --noconfirm; break ;;
            2) yay -Rns windsurf --noconfirm; break ;;
            3) yay -Rns antigravity --noconfirm; break ;;
            4) sudo pacman -Rns neovim --noconfirm && rm -rf ~/.config/nvim; break ;;
            5) break ;;
            *) echo "$MSG_INVALID" ;;
        esac
    done
...
menu_desinstalar_launcher() {
    while true; do
        ...
        read -p "$MSG_CHOOSE" op
        case "$op" in
            1) flatpak uninstall com.valvesoftware.Steam -y; break ;;
            2) yay -Rns heroic-games-launcher-bin --noconfirm; break ;;
            3) break ;;
            *) echo "$MSG_INVALID" ;;
        esac
    done
...
menu_desinstalar() {
    while true; do
        ...
        read -p "$MSG_CHOOSE" op
        case "$op" in
            1) menu_desinstalar_navegador ;;
            2) menu_desinstalar_editor ;;
            3) menu_desinstalar_launcher ;;
            4) break ;;
            *) echo "$MSG_INVALID" ;;
        esac
    done
...
while true; do
    ...
    echo "4. ⚙️ $MSG_MENU_MAIN_4"
    echo "5. 🚪 $MSG_MENU_MAIN_5"
